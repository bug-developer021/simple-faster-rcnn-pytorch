...
#  Faster RCNN 的三个核心步骤就是： 
#  1.特征提取：输入一张图片得到其特征图 feature map 
#  2. RPN：给定特征图后产生一系列 RoIs 
#  3. ROI Head：利用这些 RoIs 对应的特征图对这些 RoIs 中的类别进行分类，并提升定位精度
class FasterRCNN(nn.Module):
    ...
    # predict函数是对网络RoIhead网络输出的预处理,函数_suppress将得到真正的预测结果。
    @nograd
    def predict(self, imgs,sizes=None,visualize=False):
        ...
        for img, size in zip(prepared_imgs, sizes):
            ...
            # 对读入的图片求缩放尺度scale，因为输入的图像经预处理就会有缩放，
            # 所以需记录缩放因子scale，这个缩放因子在ProposalCreator
            # 筛选roi时有用到，原图规定了候选框 min_size, 对应到roi上就是scale*min_size,通过这个值将太小的候选框丢弃
            ...
            # 因为ProposalTargetCreator对loc做了归一化（-mean /std）处理，所以这里
            ...
            # roihead输出的roi_cls_loc的shape为(num(proposals), n_class*4)
            # roihead输出的roi_score的shape为(num(proposals), n_class)
